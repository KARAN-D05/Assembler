## 📁 Sample Programs - asm-v0
- A set of verified assembly programs written in the custom asm-v0 assembly language and tested on [Custom Computing Machine](https://github.com/KARAN-D05/Computing_Machinery_from_Scratch).

### ▸ Assembler Compatibility
| Assembly Standard | Compatible Assembler |
|---|---|
| asm-v0.1 | asm-v0.1 assembler only |
| asm-v0.2 / asm-v0.3 | asm-v0.2 or asm-v0.3 assembler |

**` It is advised to use asm-v0.3.1 assembler - includes syntax analysis, semantic analysis, flexible Program Length and execution report.`**

### ▸ Programs
| S.No. | Program | Demonstrates |
|---|---|---|
| 1 | Load & ADD | LOAD, STORE, ADD |
| 2 | Feedback | FBK, SUB, HOLD-OP |
| 3 | Division | DIV, HOLD-OP, Convergence |
| 4 | Multiplication | MUL, HOLD-OP, XXXX-1 rule |
| 5 | Overflow, fault-detection & Recovery | OVRD, overflow detection |
| 6 | Machine Reset | RSTM |
| 7 | Arithmetic Progression Generator | FBK, sequence generation |

### 🧩 Machine Code Translations
- Pre-assembled **binary files** ([Machine-Codes](Machine-Codes)) for the sample assembly programs(P1.asm - P7.asm)
